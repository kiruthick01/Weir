...
function LedgerRow({ entry }: { entry: LedgerEntry }) {
  ...
  return <div className="rounded-lg border border-zinc-800 bg-zinc-950/50 p-3"><div className="flex flex-wrap items-start gap-3"><span className="min-w-24 font-mono text-xs text-zinc-500">{new Date(entry.timestamp).toLocaleTimeString()}</span><Badge className={cn('border', outcomeStyles[entry.outcome])}>{entry.outcome}</Badge><span className="min-w-0 flex-1 text-sm leading-5 text-zinc-300">{entry.reason}</span></div>{entry.agentReasoning && <Collapsible open={open} onOpenChange={setOpen} className="mt-2 border-t border-zinc-800 pt-2"><CollapsibleTrigger className="flex items-center gap-1 text-xs text-zinc-500 hover:text-zinc-300"><ChevronDown className={cn('size-3 transition-transform', open && 'rotate-180')} /> Agent reasoning</CollapsibleTrigger><CollapsibleContent className="pt-2 font-mono text-xs leading-5 text-zinc-500">{entry.agentReasoning}</CollapsibleContent></Collapsible>}</div>
}
