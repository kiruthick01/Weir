...
export function ApproverCard({
  ...
  return (
    <button type="button" onClick={onSelect} className="block w-full text-left">
      <Card className={cn(
        'h-full border-zinc-800 bg-zinc-900/80 transition-all duration-300 hover:border-zinc-600',
        selected && 'border-cyan-400/80 ring-2 ring-cyan-400/30',
      )}>
        ...
        <CardContent className="grid grid-cols-2 gap-3 pt-0">
          <div className="rounded-lg border border-zinc-800 bg-zinc-950/70 p-3">
            <div className="flex items-center gap-2 text-xs text-zinc-500"><Inbox className="size-3.5" /> Queue</div>
            <div className="mt-1 font-mono text-xl text-zinc-100">{approver.queue.length}</div>
          </div>
          <div className="rounded-lg border border-zinc-800 bg-zinc-950/70 p-3">
            <div className="flex items-center gap-2 text-xs text-zinc-500"><Activity className="size-3.5" /> p50 latency</div>
            <div className="mt-1 font-mono text-xl text-zinc-100">{(p50 / 1000).toFixed(1)}s</div>
          </div>
          <div className="col-span-2 flex items-center gap-2 font-mono text-xs text-zinc-500">
            <Clock3 className="size-3.5" />
            {approver.pressureState === 'critical'
              ? `critical for ${criticalSeconds}s`
              : `state since ${new Date(approver.stateSince).toLocaleTimeString()}`}
          </div>
        </CardContent>
      </Card>
    </button>
